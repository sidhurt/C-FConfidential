Option Explicit
Dim rot, sapGui, app, conn, sess, w0, grid, ci, cj, found, tableName, id, r, c, colName, line, ids
...
For Each tableName In Array("E071", "E071K")
  For Each id In ids
    If sess.Children.Count <> 1 Then WScript.Echo "STOP|modal before " & tableName & " " & id : WScript.Quit 3
    sess.StartTransaction "SE16N"
    WaitReady sess
    If sess.Children.Count <> 1 Then WScript.Echo "STOP|modal after tcode " & tableName & " " & id : WScript.Quit 4
    Set w0 = sess.FindById("wnd[0]")
    sess.FindById("wnd[0]/usr/ctxtGD-TAB").Text = tableName
    w0.SendVKey 0
    WaitReady sess
    If sess.Children.Count <> 1 Then WScript.Echo "STOP|modal after table " & tableName & " " & id : WScript.Quit 5
    sess.FindById("wnd[0]/usr/tblSAPLSE16NSELFIELDS_TC/ctxtGS_SELFIELDS-LOW[2,0]").Text = id
    sess.FindById("wnd[0]/tbar[1]/btn[8]").Press
    WaitReady sess
    If sess.Children.Count <> 1 Then WScript.Echo "STOP|modal after execute " & tableName & " " & id : WScript.Quit 6
    WScript.Echo "QUERY|" & tableName & "|" & id & "|ROOT=" & sess.FindById("wnd[0]").Text & "|SBAR=" & sess.FindById("wnd[0]/sbar").Text

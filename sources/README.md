...
| `SRC-DOC-20260910-01` | `SRC-DOC-20260910-01_CNF_T2_Field_Mapping_T1_v1.2.xlsx` | Functional-architect T2 field-mapping workbook. The `completed MRN` sheet defines a composite Commerce/Datasphere projection led by `VW_CDS_MATDOC_MATERIALDOCUMENT`, with ILMS and master-data enrichments | **Controlled client mapping; not runtime proof.** Review in `../sessions/2026-09-10-completed-mrn-mapping-review/FINDINGS.md`. SHA-256 `311828436277FB3CCEA8E3FE878CDE8C17FA1F5D0BA3C1DD62FE5BEBEE34FED6` |
...
| `SRC-SID-20260910-01` | `SRC-SID-20260910-01_Batched_Read_Model_Clarification.md` | Direct clarification that SAP/Datasphere display data is transmitted through CPI in batches, that portal screens use a persisted read model rather than per-interaction SAP reads, and that the MIGO cadence is 30 minutes | Direct project clarification; confirms MIGO cadence and read/command separation. Exact trigger, interface and reliability design remain open |
| `SRC-SID-20260910-02` | `SRC-SID-20260910-02_Existing_CPI_T1_Delivery_Enrichment.md` | Direct clarification that batch processing and field/table relationships are already filled in CPI/T1 by delivery number; SAP transactional APIs are not responsible for rebuilding the full screen context | Direct project-state clarification; existing implementation still requires runtime trace and exact T1/T2 persistence identification |
| `SRC-SID-20260914-01` | `SRC-SID-20260914-01_ZLE_SHP_DELIVERY_PROC_Source_Ownership.md` | Direct confirmation that `methods for ZLE_SHP_DELIVERY_PROC.txt` contains source from BAdI implementation `ZLE_SHP_DELIVERY_PROC` | Source ownership verified by Siddharth; execution on OData/BAPI remains a separate runtime question |
| `SRC-SID-20260915-01` | `SRC-SID-20260915-01_Create_DI_Candidate_Consumption.md` | Direct report that STO `5600084274/00010` was consumed by the SAP Gateway Client Create DI request | Candidate consumption recorded; created delivery number and independent persistence readback remain open |
| `SRC-SID-20260915-02` | `SRC-SID-20260915-02_Create_DI_Validation_Review_Feedback.md` | Stakeholder feedback relayed by Siddharth: distinguish the two SLoc controls, scope wheeler validations 4–6 to Primary Plant, discuss the SPI rule, prove API execution, and inspect all enhancement types | Reported review direction; original controlled email/message and approver identity not held |
| `SRC-SID-20260915-03` | `SRC-SID-20260915-03_Depot_Prerequisite_Function_and_Lifecycle.md` | User-supplied `ZLEF_DELIVERY_VALIDATONS` source and lifecycle clarification for Create DI, Pre-PGI enrichment, PGI and STO receipt | Function rules verified from normalized supplied source; exact SAP export, message texts, `UPDATE_LOG`, other callers and API runtime remain open |
| `SRC-SID-20260915-04` | `SRC-SID-20260915-04_Gateway_Create_DI_HEAD_No_Write.md` | Gateway Client attempt for `5600084222/00010` used `HEAD` on the service root and returned empty HTTP 200; it was not a Create DI POST | No-write conclusion is source-backed; candidate availability still requires current SAP re-read |
| `SRC-SID-20260917-06` | `SRC-SID-20260917-06_CNF_Create_DI_Depot_Scope.md` | C&F Create DI covers both depot → customer sales-order dispatch and depot → depot STO deliveries | Scope stated directly; BTST/`ZZVBELN` exclusion and depot STO delivery type remain to confirm |
| `SRC-SID-20260917-07` | `SRC-SID-20260917-07_Create_DI_Test_Tool_Gateway_Client.md` | Create DI testing uses `/IWFND/GW_CLIENT`, not Postman | Tool choice stated directly; request templates in the Create DI test approach |
| `SRC-SID-20260917-08` | `SRC-SID-20260917-08_Hybris_Enrichment_Ownership_and_Duplicate_Blocking.md` | SAP must build an API that receives transporter, storage location/batch and other enrichment fields from Hybris T2; SAP→Hybris master data is a near-real-time sync and SAP stays authoritative; Hybris or CPI blocks duplicate DIs | Field list, write mechanism and step boundaries of the enrichment API to be designed; lost-response retry handling not covered |
| `SRC-SID-20260917-09` | `SRC-SID-20260917-09_Create_DI_Portal_Screen.md` | Create DI portal modal: one order line displayed read-only; the only input is Delivery Quantity | Illustrative design values; shipping-point mapping and pending-quantity source not shown. Supersedes the MIGO image as Create DI context |
| `SRC-SID-20260917-10` | `SRC-SID-20260917-10_Create_DI_Block_and_BTST_Requirements.md` | Business requires Create DI to apply delivery block, billing block and bill-to-ship-to controls | Relayed answer; which location raises BTST DIs not stated |
| `SRC-SID-20260921-01` | `SRC-SID-20260921-01_Modify_DI_Scope_and_Portal_Flow.md` | Modify DI is required (SD consultant); portal Figma flow and field list after Create DI; transporter name/address from T2/CPI master | Figma is a design; FTP/FTB and 30.5/30 MT inconsistencies |
| `SRC-SID-20260921-02` | `SRC-SID-20260921-02_SPI_Pickup_Code_and_Commit_Point.md` | SPI auto-populated from master/order; pick-up code from T1/Udaan; everything resubmittable until "Generate Invoice & E-Way Bill", final after | Whether billing/e-way reads LIKP-PICKUPCODE not verified |
| `SRC-SID-20260921-03` | `SRC-SID-20260921-03_Create_DI_QA_Results.md` | QA: 8 validations passed, 3 failed (future schedule line, deletion flag, error message); VL01N shows `VL 248`; VL01N has no deletion-flag check | Test documents reconstructed from change logs |
| `SRC-SID-20260917-01` | `SRC-SID-20260917-01_Submit_MIGO_Phase_1_QS4_Status.md` | Direct report that Submit MIGO phase-1 validation logic, including the delivery-led/no-PO case, is implemented in `ZCL_MM_GOODSMVT_BAPI_CUSTOM` under `MB_BAPI_GOODSMVT_CREATE` and works in QS4 | Team sequencing fact; active source, executed payload and independent persistence/replay evidence still need capture before full certification |
| `SRC-SID-20260917-02` | `SRC-SID-20260917-02_MIGO_Status_Correction_and_Create_DI_Test_Focus.md` | Corrects the MIGO status to partial validation implementation and confirms the seven-rule Create DI validation scope plus test-first direction | Supersedes any reading of `SRC-SID-20260917-01` as phase-1 completion; Create DI API behavior remains to be runtime-certified rule by rule |

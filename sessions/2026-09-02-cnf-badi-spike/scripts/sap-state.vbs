Option Explicit
' Read-only state probe for a named system/client.
' USAGE: sap-state.vbs <SYS> <CLIENT> [DUMP]
Dim app, conn, s, sess, found, i, j, wantSys, wantCli, doDump

If WScript.Arguments.Count < 2 Then WScript.Echo "USAGE|SYS CLIENT [DUMP]" : WScript.Quit 1
wantSys = UCase(CStr(WScript.Arguments(0)))
wantCli = CStr(WScript.Arguments(1))
doDump = (WScript.Arguments.Count > 2)
...
WScript.Echo "SYSTEM=" & sess.Info.SystemName & "/" & sess.Info.Client & " USER=" & sess.Info.User
...
Sub Dump(node, depth)
  ...
  WScript.Echo Space(depth) & Replace(node.Id, "/app/con[" & i & "]/ses[" & j & "]/wnd[0]/usr", "") & " [" & t & "] '" & Left(nm, 45) & "'"

Option Explicit
' READ-ONLY SE16 reader, system/client parameterised.
' USAGE: sap_se16_read.vbs <SYS> <CLIENT> <TABLE> <MAXROWS> <COLS_CSV> [FIELD=LOW[..HIGH]] ...
Dim app, conn, s, sess, found, i, j, tbl, maxRows, colsCsv, cols, elapsed
Dim lbl, n, nameOf, arg, eq, fn, fv, hi, k, g, r, c, line, v, dumped, wantSys, wantCli

If WScript.Arguments.Count < 5 Then WScript.Echo "USAGE|SYS CLIENT TABLE MAXROWS COLS [F=V]" : WScript.Quit 1
wantSys = UCase(CStr(WScript.Arguments(0)))
wantCli = CStr(WScript.Arguments(1))
tbl     = UCase(CStr(WScript.Arguments(2)))
maxRows = CLng(WScript.Arguments(3))
colsCsv = UCase(CStr(WScript.Arguments(4)))
cols    = Split(colsCsv, ",")
...
Next
If Not found Then WScript.Echo "ABORT|NO_" & wantSys & "_" & wantCli : WScript.Quit 9
...
For i = 5 To WScript.Arguments.Count - 1
  ...
  If eq > 1 Then
    ...
    If nameOf.Exists(fn) Then
      ...
      If Not SetField("wnd[0]/usr/ctxtI" & n & "-LOW", fv) Then
        If Not SetField("wnd[0]/usr/txtI" & n & "-LOW", fv) Then WScript.Echo "WARN|CANNOT_SET|" & fn
      End If
      If Len(hi) > 0 Then
        If Not SetField("wnd[0]/usr/ctxtI" & n & "-HIGH", hi) Then SetField "wnd[0]/usr/txtI" & n & "-HIGH", hi
      End If
      WScript.Echo "FILTER|" & fn & "(I" & n & ")=" & fv
    Else
      WScript.Echo "ABORT|FIELD_NOT_ON_SCREEN|" & fn & "|available=" & Join(nameOf.Keys, ",")
      WScript.Quit 15
    End If
  End If
Next

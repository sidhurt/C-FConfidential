Option Explicit
' STRICTLY READ-ONLY SE16 reader for QS4 client 700.
' Locates the QS4/700 session by system name, never by index, and refuses anything else.
...
'
' Usage: cscript //nologo qs4_se16_read.vbs <TABLE> <MAXROWS> [selFieldId=VALUE ...]

Dim sapGuiAuto, app, sess, elapsed
Dim tbl, maxRows, i, j, arg, eq, fid, fval, conn, s, found

If WScript.Arguments.Count < 2 Then
  WScript.Echo "USAGE|qs4_se16_read.vbs <TABLE> <MAXROWS> [selFieldId=VALUE ...]"
  WScript.Quit 1
End If
...
For i = 0 To app.Children.Count - 1
  ...
  For j = 0 To conn.Children.Count - 1
    ...
    If s.Info.SystemName = "QS4" And s.Info.Client = "700" Then
      Set sess = s
      found = True
      WScript.Echo "SESSION|QS4/700 located at connection[" & i & "] session[" & j & "]|user=" & s.Info.User
      Exit For
    End If
  Next
...
If Not found Then
  WScript.Echo "ABORT|NO_QS4_700_SESSION"
  WScript.Quit 9
End If
...
' QS4 renders the SE16 result as an ALV grid rather than a classic list.
